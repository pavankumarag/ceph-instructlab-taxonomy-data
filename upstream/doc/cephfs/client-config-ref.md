...
Certain client configurations can be applied at runtime. To check if a
configuration option can be applied (taken into affect by a client) at
runtime, use the [config help]{.title-ref} command:
...
[config help]{.title-ref} tells if a given configuration can be applied
at runtime along with the defaults and a description of the
configuration option.

To update a configuration option at runtime, use the [config
set]{.title-ref} command:

    ceph config set client debug_client 20/20

Note that this changes a given configuration for all clients.

To check configured options use the [config get]{.title-ref} command:
...
These options are internal. They are listed here only to complete the
list of options.
::::
