...
Full path to a raw device, like `/dev/sda`. Multiple `<DEVICE>` paths
can be passed in.
:::
...
Prepares a logical volume to be used as an OSD and journal using a
...
Attaches the given logical volume to OSD as a WAL. Logical volume name
