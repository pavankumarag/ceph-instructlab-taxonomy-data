...
What should be the refcount for `aaa` be at the end? By our above rule,
...
-   `evict-chunk`
    ...
    Clears an extent from an object leaving only the manifest link
...
-   `tier-flush`
    ...
    Included in the `user_modify=false` `ignorelist`, does not trigger a
    clone.
